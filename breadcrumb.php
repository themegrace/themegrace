...
if ( ! function_exists( 'themegrace_current_url' ) ) :
	/**
	 * Get Current URL
	 *
	 * @return string Current url
	 */
	function themegrace_current_url() {
		static $url = null;
		if ( null === $url ) {
			$url = 'http://';
...
class Themesurge_Breadcrumbs {
	...
	public function __construct( $settings = array() ) {
		$homepage_title = __( 'Home', 'bloggingpro' );
		$blog_title     = __( 'Blog', 'bloggingpro' );
		$error_title    = __( '404 Not found', 'bloggingpro' );
	...
	private function build_breadcrumbs() {
		...
		if ( is_404() ) {
			...
			$page['url']  = themegrace_current_url();
			$return[]     = $page;
		} elseif ( is_search() ) {
			$search         = array();
			$search['type'] = 'search';
			$search['name'] = __( 'Searching for:', 'bloggingpro' ) . ' ' . get_search_query();
		...
		foreach ( $return as $key => $item ) {
			if ( empty( $item['name'] ) ) {
				$return[ $key ]['name'] = __( 'No title', 'bloggingpro' );
			}
		}
		return $return;
	}
...
if ( ! function_exists( 'themegrace_view_breadcrumbs' ) ) :
	/**
	 * Displaying html for breadcrumb
	 *
	 * @return void
	 */
	function themegrace_view_breadcrumbs() {

		$breadcrumbs   = new Themesurge_Breadcrumbs();
		$data['items'] = $breadcrumbs->get_breadcrumbs();

		$items = $data['items'];

		if ( ! is_front_page() && ! is_home() && ! empty( $items ) ) :
			$opsibreadcrumbs = get_theme_mod( 'gmr_active-breadcrumb', 0 );
			if ( 0 === $opsibreadcrumbs ) :
				?>
				...
				for ( $i = 0; $i < $countitems; $i ++ ) :
					...
					<?php if ( ( count( $items ) - 1 === $i ) ) : ?>
						...
						<?php else : ?>
							<?php if ( isset( $items[ $i ]['url'] ) ) : ?>
								<span class="<?php echo esc_attr( $i - 1 ); ?>-item" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
									<a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="<?php echo esc_url( $items[ $i ]['url'] ); ?>" href="<?php echo esc_url( $items[ $i ]['url'] ); ?>">
										<span itemprop="name"><?php echo esc_html( $items[ $i ]['name'] ); ?></span>
									</a>
									<span itemprop="position" content="<?php echo esc_html( $i + 1 ); ?>"></span>
...
add_action( 'themegrace_view_breadcrumbs', 'themegrace_view_breadcrumbs', 10 );
